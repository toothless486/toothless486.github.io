---
layout: post
title: "[Lock 디버깅 #1] MQ 연결이 끊기는 이유 - Blocking I/O와 Lock 경합"
description: "C 멀티스레드 환경에서 Blocking I/O와 Mutex Lock의 조합이 MQ Heartbeat를 지연시켜 연결을 끊는 원인을 분석하고, 버퍼링으로 Lock 보유 시간을 40초에서 500ms로 줄여 해결한 트러블슈팅 기록입니다."
...
>
> **대상 독자**: C 언어와 멀티스레드 프로그래밍(pthread)에 익숙한 독자를 대상으로 합니다.
...
실제 로그에서 패턴이 명확하게 드러난다.

```
# 아래 로그는 재현 테스트 환경에서 수집한 것으로, 실제 값은 가공되었습니다.

[00:04:44.679][tid 1001] [MQ] ->. Send HeartBeat
[00:04:44.679][tid 1002] [MQ] ->. Lock Read Thread(stomp_read)
[00:04:44.973][tid 1001] [MQ] HEARTBEAT THREAD (count: 1)   ← Lock 대기 중, 전송 불가
[00:04:45.974][tid 1001] [MQ] HEARTBEAT THREAD (count: 2)
[00:04:46.975][tid 1001] [MQ] HEARTBEAT THREAD (count: 3)
[00:04:47.976][tid 1001] [MQ] HEARTBEAT THREAD (count: 4)
[00:04:48.976][tid 1001] [MQ] HEARTBEAT THREAD (count: 5)
[00:04:49.681][tid 1002] [MQ] ->. Unlock Read Thread(stomp_read)
[00:04:49.681][tid 1002] [MQ] stomp_read section took 5001 ms  ← Read Thread가 5초간 Lock 점유

... (이 패턴이 반복됨)

[00:05:04.027][tid 1002] [MQ] stomp_read section took 4342 ms
[00:05:04.027][tid 1002] [MQ] RC: <ETIMEDOUT>               ← 연결 타임아웃
[00:05:04.027][tid 1002] [MQ]   + Socket Closed
[00:05:04.027][tid 1002] [MQ]   + STOMP DISCONNECTED
```

`HEARTBEAT THREAD (count: N)`은 Heartbeat Thread가 매초 카운터를 올리는 로그다. Read Thread(tid 1002)가 Lock을 잡고 있는 동안 Heartbeat Thread(tid 1001)는 실제 Heartbeat를 전송하지 못한다. 결국 서버는 Heartbeat가 오지 않는다고 판단해 연결을 끊는다.
...
while (running) {
    ...
    rc = mq_read_frame(conn, &frame, pool);  // pool: 프레임 데이터 할당용 메모리 풀
...
int read_and_process(void) {
    ...
    rc = recv(socket, temp_buffer, recv_len, 0);         // rc: 수신 바이트 수, -1이면 에러
...
- **단위 테스트 추가**: `read_and_process()`를 `TESTABLE`(`static` 키워드를 테스트 빌드에서만 제거하는 매크로)로 변경하여 테스트 가능하도록 개선
...
- [`recv(2)` - Linux man page](https://man7.org/linux/man-pages/man2/recv.2.html)
- [`pthread_mutex_lock(3)` - Linux man page](https://man7.org/linux/man-pages/man3/pthread_mutex_lock.3p.html)

---

> 이 글은 [동시성](/categories/동시성/) 시리즈의 일부입니다.
